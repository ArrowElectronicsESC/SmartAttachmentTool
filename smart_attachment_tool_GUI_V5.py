...
if len(sys.argv) <= 1:
    print("Usage: python3 smart_attachment_tool.py <BluetoothAntenna-file-name>")
else:
    # pxl_doc = openpyxl.load_workbook("./BluetoothAntenna20220704075829.xlsx")
    pxl_doc = openpyxl.load_workbook(sys.argv[1])
    ...
    for i in range(2, sheet.max_row+1):
        ...
        if bt is not None:
            ...
            with open(f"./output_excel/{bluetooth}_compatible_parts.csv", "w") as f:
                f.write("Adhesive,Chasis Mount,Panel Mount,Surface Mount\n")
                for i in range(3):
                    row = ""
...
#         self.labelSurface.move(380, 180)
